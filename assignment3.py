...
class Assignment3:
    ...
    def integrate(self, f: callable, a: float, b: float, n: int) -> np.float32:
        ...
        for i in range(1, new_n):
            ...
            if i % 2 == 0:
                ...
                sum_of_odd += f(x)
        ans = (delta/3) * (f(a) + 4*sum_of_odd + 2*sum_of_even + f(b))
    ...
    def areabetween(self, f1: callable, f2: callable) -> np.float32:
        ...
        def func_f1_min_f2(x):
            return f1(x)-f2(x)

        def func_f2_min_f1(x):
            return f2(x)-f1(x)

        s2 = assignment2.Assignment2()
        points = s2.intersections(f1, f2, 1, 100)

        def count_iterable(it):
            return sum(1 for e in it)

        counter = count_iterable(points)
        ...
        while i < counter - 1:
            if f1(points[i]) < f2(points[i]):
                tot_area += self.integrate(func_f2_min_f1, points[i], points[i + 1], j)
            else:
                tot_area += self.integrate(func_f1_min_f2, points[i], points[i + 1], j)
            i += 1
        result = np.float32(tot_area)
